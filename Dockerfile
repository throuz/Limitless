# polymkt mm-loop 容器映像
#
# 設計：
# - python:3.13-slim 基底,小、快
# - 把 polymkt 安裝成 editable package(含 [trade]),所以 limitless-sdk 進來
# - 用 tini 當 PID 1 → 轉發 SIGTERM 給 polymkt → graceful shutdown 工作
...
COPY polymkt ./polymkt

# 含 trade extra(limitless-sdk)
RUN pip install -e '.[trade]'

# 不以 root 跑(基本 hardening)
RUN useradd --create-home --shell /bin/bash polymkt
USER polymkt

# Healthcheck:輸出能否取得活躍市場
HEALTHCHECK --interval=5m --timeout=30s --start-period=60s --retries=3 \
    CMD python -c "import asyncio; from polymkt.limitless.client import LimitlessClient; \
asyncio.run((lambda: LimitlessClient().fetch_active_markets(max_markets=1))()) " || exit 1

ENTRYPOINT ["tini", "--", "python", "-m", "polymkt.cli"]
